...
export function initState (vm: Component) {
  ...
  if (opts.computed) initComputed(vm, opts.computed) // 初始化，就一用到这个vue实例，就去判断是否有computed
  // 初始化，就一用到这个vue实例，就去判断是否有watch
...
function initComputed (vm: Component, computed: Object) {
  ...
  for (const key in computed) {
    ...
    if (!isSSR) {
      ...
      // 每个 computed 都创建一个 watcher,watcher 用来存储计算值，判断是否需要重新计算
