...
* 相关参数说明
  * 第一个参数是侦听器的源（一个函数，返回一个值/一个 ref/一个响应式对象，...或是由以上类型的值组成的数组）
  * 第二个参数是在发生变化时要调用的回调函数。这个回调函数接受三个参数：新值、旧值，__以及一个用于注册副作用清理的回调函数__。该回调函数会在副作用下一次重新执行前调用，可以用来清除无效的副作用，例如等待中的异步请求
...
* 相关代码：
  * 示例1
    ...
    watch(message, (newVal, oldVal) => {
        ...
        immediate:true, // 立即执行
        deep:true // 针对于ref的响应式对象，reactive不需要
    })
    ```

 同时发现一个问题，新值和旧值是一样的，通过源码可知，引用类型是复制过去的，所以新值和旧值一样
...
* 注意点
    ...
    2. 监听reactive数据源使用提醒:
        ...
        2. 不可以直接侦听响应式对象的属性值(因为newVal和oldVal都是同一个值) __ref也是如此__，只能通过返回该属性的 getter 函数或者转为deep：true(示例4)
...
* 大白话：立即运行一个函数，同时响应式地追踪其依赖，并在依赖更改时重新执行
* 相关代码
  * 示例1
  ...
  * 示例2
    ...
    oninvalidate回调函数的意思就是当message/obj更新时会先触发oninvalidate，然后再打印console，这个无关于顺序，在这个oninvalidate回调中我们可以在更新message/obj前去做一些其他的事情
  ...
  * 示例3
    ...
    就是通过事件来清除watchEffect，这样的话就不会再监听message/obj了，但是oninvalidate依然会触发
  ...
  * 示例4
    ...
    watchEffect会一进页面就会触发，所以这里面dom是无法获取的，可以设置第三个参数flush：post,这样会在页面加载完后触发watchEffect
